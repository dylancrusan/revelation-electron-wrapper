...
const { MANIFEST_FILENAME, buildPresentationManifest, writePresentationManifest } = require('./presentationManifest');
...
// Mirror of CSS_VERSION_SNAPSHOTS from revelation/js/compiler/compiler-utils.js.
// Keep in sync whenever a new snapshot folder is added there.
const CSS_VERSION_SNAPSHOTS = [
  { folder: '1.0.6', breakpoint: [1, 0, 7] },
  // { folder: '1.1.0', breakpoint: [1, 1, 1] },
];

function resolveFirstExistingPath(candidates, description) {
  for (const candidate of candidates) {
    if (!candidate) continue;
    if (fs.existsSync(candidate)) return candidate;
  }
  throw new Error(`Missing ${description}. Checked: ${candidates.filter(Boolean).join(', ')}`);
}
...
function createOfflineHTML(
  ...
  const remoteServerLiteral = JSON.stringify(String(remoteServer || ''));
  const presenterPluginsServerLiteral = JSON.stringify(String(presenterPluginsServer || ''));
...
<head>
  ...
  <script>
    window.revealRemoteServer = ${remoteServerLiteral};
    window.presenterPluginsPublicServer = ${presenterPluginsServerLiteral};
    ${mediaLink}
    ${versionLine}
  </script>
  ${headTags}
...
// Returns the oldcss folder name needed for a presentation's metadata version,
// or null if current CSS should be used.  Mirrors resolveLegacyCssFolder() in
// revelation/js/compiler/compiler-utils.js — keep both in sync.
function resolveLegacyCssFolder(metadata = {}) {
...
// Returns the deduplicated set of oldcss folder names needed by a list of
// presentation objects (each having a `content` property with raw markdown).
function collectOldCssVersionsNeeded(presentations) {
  const needed = new Set();
  for (const pres of presentations) {
    const frontMatter = parseFrontMatter(pres.content);
    const folder = resolveLegacyCssFolder(frontMatter);
    if (folder) needed.add(folder);
  }
  return needed;
}
...
const exportPresentation = {
  ...
  presenterPluginsServer: null, // to be set in register
  AppCtx: null, // to be set in register

  // Register IPC handlers
  register(ipcMain, AppContext) {
    this.AppCtx = AppContext;
    ipcMain.handle('export-presentation', async (event, slug, includeMedia, options = {}) => {
      const folderPath = path.join(AppContext.config.presentationsDir, slug);
      const showSplashscreen = options?.showSplashscreen !== false;
      const useRevealRemotePublicServer = options?.useRevealRemotePublicServer !== false;
      this.remoteServer = useRevealRemotePublicServer ? AppContext.config.revealRemotePublicServer : '';
      this.presenterPluginsServer = useRevealRemotePublicServer ? AppContext.config.presenterPluginsPublicServer : '';
  ...
  async run(folderPath, slug, includeMedia, showSplashscreen, event) {
    ...
    const revelationRoot = this.AppCtx?.config?.revelationDir || REVELATION_ROOT;
    const cssSrc = resolveFirstExistingPath(
      [
        path.join(revelationRoot, 'dist', 'css'),
        path.join(process.resourcesPath || '', 'revelation', 'dist', 'css'),
        path.join(REVELATION_ROOT, 'dist', 'css')
      ],
      'theme CSS directory (revelation/dist/css)'
    );
    ...
    try {
      ...
      const mdFiles = collectMarkdownFilesRecursive(folderPath);
      if (mdFiles.length === 0) {
      ...
      // Copy only the oldcss snapshot version(s) actually used by these presentations.
      const oldCssVersionsNeeded = collectOldCssVersionsNeeded(presentations);
      for (const version of oldCssVersionsNeeded) {
        const oldCssSrc = path.join(revelationRoot, 'assets', 'oldcss', version);
        if (!fs.existsSync(oldCssSrc)) {
          console.warn(`⚠️ oldcss snapshot missing for version ${version}: ${oldCssSrc}`);
          continue;
        }
        const oldCssDest = path.join(resourcesDir, 'oldcss', version);
        fs.mkdirSync(oldCssDest, { recursive: true });
        fs.cpSync(oldCssSrc, oldCssDest, { recursive: true, force: true });
        console.log(`✓ Included oldcss/${version} in export`);
      }
      const jsSrc = resolveFirstExistingPath(
        [
          path.join(revelationRoot, 'dist', 'js', 'offline-bundle.js'),
          path.join(process.resourcesPath || '', 'revelation', 'dist', 'js', 'offline-bundle.js'),
          path.join(REVELATION_ROOT, 'dist', 'js', 'offline-bundle.js')
        ],
        'offline bundle (revelation/dist/js/offline-bundle.js)'
      );
      fs.copyFileSync(jsSrc,path.join(resourcesDir,'offline-bundle.js'));
      const rcssSrc = resolveFirstExistingPath(
        [
          path.join(revelationRoot, 'node_modules', 'reveal.js', 'dist', 'reveal.css'),
          path.join(process.resourcesPath || '', 'revelation', 'node_modules', 'reveal.js', 'dist', 'reveal.css'),
          path.join(REVELATION_ROOT, 'node_modules', 'reveal.js', 'dist', 'reveal.css')
        ],
        'reveal.css (revelation/node_modules/reveal.js/dist/reveal.css)'
      );
      fs.copyFileSync(rcssSrc,path.join(resourcesDir,'css','reveal.css'));
      const translationsSrc = resolveFirstExistingPath(
        [
          path.join(revelationRoot, 'js', 'translations.json'),
          path.join(process.resourcesPath || '', 'revelation', 'js', 'translations.json'),
          path.join(REVELATION_ROOT, 'js', 'translations.json')
        ],
        'translations.json (revelation/js/translations.json)'
      );
      fs.copyFileSync(translationsSrc, path.join(resourcesDir, 'translations.json'));
      const translateSrc = resolveFirstExistingPath(
        [
          path.join(revelationRoot, 'js', 'translate.js'),
          path.join(process.resourcesPath || '', 'revelation', 'js', 'translate.js'),
          path.join(REVELATION_ROOT, 'js', 'translate.js')
        ],
        'translate.js (revelation/js/translate.js)'
      );
      ...
      fs.writeFileSync(path.join(folderPath, MANIFEST_FILENAME), JSON.stringify(manifest, null, 2), 'utf-8');
      ...
      if (fs.existsSync(resourcesDir)) {
        // Retry on ENOTEMPTY — Windows may report a non-empty directory while
        // still flushing recently-deleted files (race condition).
        let removed = false;
        for (let attempt = 0; attempt < 5 && !removed; attempt++) {
          ...
          try {
            ...
            if (err.code !== 'ENOTEMPTY' || attempt === 4) {
              console.warn(`[exportPresentation] Failed to remove _resources: ${err.message}`);
            }
          }
        }
      }
      if (fs.existsSync(folderPath)) {
        try {
          const markdownFiles = collectMarkdownFilesRecursive(folderPath);
          writePresentationManifest(folderPath, {
            appVersion: app.getVersion(),
            savedAt: new Date().toISOString(),
            markdownFiles
          });
        } catch (err) {
          console.warn(`[exportPresentation] Failed to write manifest after export: ${err.message}`);
        }
      }
    }
  }
