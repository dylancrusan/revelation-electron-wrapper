...
function handlePeerCommand(AppContext, command, sourceMasterId = 'unknown') {
  ...
  switch (command.type) {
    ...
    case 'navigate-slide': {
      const { h, v } = command.payload || {};
      const safeH = Number.isFinite(Number(h)) ? Number(h) : 0;
      const safeV = Number.isFinite(Number(v)) ? Number(v) : 0;
      if (!presentationWindow.presWindow || presentationWindow.presWindow.isDestroyed()) break;
      presentationWindow.presWindow.webContents
        .executeJavaScript(`window.deck && window.deck.slide(${safeH}, ${safeV})`)
        .catch((err) => AppContext.error(`navigate-slide failed: ${err.message}`));
      break;
    }
    ...
    default: {
      // Plugin extension point: plugins call AppContext.pluginPeerCommandHandlers.set(type, fn)
      // in their register() hook to handle custom peer command types without modifying this file.
      // Falls back to a log warning when no plugin claims the command.
      const handler = AppContext.pluginPeerCommandHandlers?.get(command.type);
      if (handler) {
        try {
          handler(command, sourceMasterId);
        } catch (err) {
          AppContext.error(`[peer-command] Plugin handler error for "${command.type}": ${err.message}`);
        }
      } else {
