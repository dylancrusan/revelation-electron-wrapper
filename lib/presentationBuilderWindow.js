...
const presentationBuilderWindow = {
  ...
  open(AppContext, slug, mdFile) {
    ...
    const builderWin = new BrowserWindow({
      width: 1800,
      height: 960,
      // Matches builder.css's body.builder background — without this the
      // window compositor's own default (white) shows through until the
      // page finishes painting over it, on both open and close.
    ...
    if (AppContext.config.runtimeEnableDevTools) {
