...
function buildInternalPresentationUrl(AppContext, slug, mdFile, overrides = {}) {
  ...
  const lang = String(requestedLanguage || '').trim().toLowerCase();
  const variant = String(requestedVariant || '').trim().toLowerCase();
  const ccli = getConfiguredCcliNumber(AppContext.config);
  if (lang) {
  ...
  if (Number.isInteger(overrides.indexh) && Number.isInteger(overrides.indexv)) {
    presentationUrl = `${presentationUrl}#/${overrides.indexh}/${overrides.indexv}`;
  }
...
function addCacheBustParam(rawUrl, revision) {
  try {
    const parsed = new URL(rawUrl);
    parsed.searchParams.set('_pubrev', String(revision || '0'));
    return parsed.toString();
  } catch {
    const joiner = String(rawUrl || '').includes('?') ? '&' : '?';
    return `${rawUrl}${joiner}_pubrev=${encodeURIComponent(String(revision || '0'))}`;
  }
}
...
const presentationWindow = {
    ...
    register(ipcMain, AppContext) {
        ...
        // Renderer sends this after electronAPI.fadeToBlack() or a
        // 'presentation-fade-to-black-request' IPC message completes.
        ipcMain.on('presentation-fade-to-black-done', (event) => {
            if (!this.presWindow || event.sender !== this.presWindow.webContents) return;
            if (typeof this._fadeToBlackResolve === 'function') {
                const resolve = this._fadeToBlackResolve;
                this._fadeToBlackResolve = null;
                resolve();
            }
        });
    },

    // Ask the presentation window to fade to black and return a Promise that
    // resolves once the screen is fully black (or immediately if no window open).
    // Usage: await presentationWindow.requestFadeToBlack(); then close.
    requestFadeToBlack(timeoutMs = 1000) {
        return new Promise((resolve) => {
            if (!this.presWindow) { resolve(); return; }
            this._fadeToBlackResolve = resolve;
            this.presWindow.webContents.send('presentation-fade-to-black-request');
            // Safety timeout in case the renderer never responds.
            setTimeout(() => {
                if (typeof this._fadeToBlackResolve === 'function') {
                    this._fadeToBlackResolve = null;
                    resolve();
    ...
    async openWindow(AppContext, slug, mdFile, fullscreen, overrides = {}) {
      ...
      this.presWindow.webContents.on('did-create-window', (childWindow, details) => {
        ...
        childWindow.webContents.on('dom-ready', () => {
          childWindow.webContents.executeJavaScript(`
            const notesLabel = document.querySelector('.speaker-controls-notes .label');
            if (notesLabel) notesLabel.textContent = 'Presenter Notes';

            function applyIframeBorder() {
              document.querySelectorAll('#current-slide iframe, #upcoming-slide iframe').forEach(el => {
                el.style.setProperty('border', '1px solid #aaaaaa', 'important');
              });
            }
            applyIframeBorder();
            new MutationObserver(applyIframeBorder).observe(document.body, { childList: true, subtree: true, attributes: true, attributeFilter: ['style', 'src'] });
          `);
          childWindow.webContents.insertCSS(`
            body { background: #1a1a1a !important; color: #e0e0e0 !important; }
            #connection-status { background: #1a1a1a !important; color: #e0e0e0 !important; }
            .speaker-controls-time .label,
            .speaker-controls-pace .label { color: #aaa !important; }
            .speaker-controls-time .reset-button { color: #aaa !important; }
            #speaker-layout { color: #e0e0e0 !important; }
            .overlay-element { background: rgba(60,60,60,0.8) !important; color: #e0e0e0 !important; }
            #current-slide iframe, #upcoming-slide iframe { border: 1px solid #aaaaaa !important; }
            #speaker-controls { display: flex !important; flex-direction: column !important; overflow: hidden !important; }
            .speaker-controls-time, .speaker-controls-pace { flex-shrink: 0; }
            .speaker-controls-notes { flex: 1 !important; overflow-y: auto !important; min-height: 0 !important; background: #fff !important; }
            .speaker-controls-notes .label { color: #666 !important; }
            .speaker-controls-notes .value { color: #222 !important; }
          `);
        });
      ...
      this.presWindow.on('close', async (event) => {
        if (this._fadingBeforeClose) return; // already mid-fade, let it proceed
        event.preventDefault();
        this._fadingBeforeClose = true;
        await this.requestFadeToBlack();
        this._fadingBeforeClose = false;
        if (this.presWindow && !this.presWindow.isDestroyed()) {
          this.presWindow.destroy(); // destroy() bypasses the close event
        }
      });
    ...
    getPublishEntries(additionalScreens = []) {
      return (Array.isArray(additionalScreens) ? additionalScreens : []).filter((entry) => entry?.target === 'publish');
    },

    getPublishFileInfo(AppContext) {
      const key = String(AppContext?.config?.presentationPublishKey || '').trim();
      if (!key) return null;
      const safeKey = key.replace(/[^a-zA-Z0-9_-]/g, '');
      if (!safeKey) return null;
      const relativePath = path.join('publish', `${safeKey}.html`);
      const revRelativePath = path.join('publish', `${safeKey}.rev`);
      const filePath = path.join(app.getPath('userData'), relativePath);
      const revFilePath = path.join(app.getPath('userData'), revRelativePath);
      const host = AppContext.hostLANURL || AppContext.hostURL || 'localhost';
      const url = `http://${host}:${AppContext.config.viteServerPort}/${relativePath.replace(/\\/g, '/')}`;
      const revUrl = `http://${host}:${AppContext.config.viteServerPort}/${revRelativePath.replace(/\\/g, '/')}`;
      return {
        key: safeKey,
        filePath,
        revFilePath,
        relativePath: relativePath.replace(/\\/g, '/'),
        revRelativePath: revRelativePath.replace(/\\/g, '/'),
        url,
        revUrl
      };
    },
    ...
    resolveDefaultPublishedTarget(AppContext, publishConfig = {}) {
      const mode = this.getScreenDefaultMode(publishConfig) || this.getVirtualPeersDefaultMode(AppContext?.config || {});
      if (mode === 'presentation') {
        const localPath = String(publishConfig.defaultPresentation || '').trim();
        const parsed = this.parseDefaultPresentationPath(localPath || AppContext?.config?.virtualPeersDefaultPresentation || '');
        if (parsed) {
          const url = buildInternalPresentationUrl(AppContext, parsed.slug, parsed.mdFile, {
            language: publishConfig.language || (AppContext.config.preferredPresentationLanguage || AppContext.config.language || 'en'),
            variant: publishConfig.variant || ''
          });
          return { type: 'url', value: url };
        }
        AppContext.log('URL Publish default presentation path invalid; using solid black default screen.');
        return { type: 'color', value: '#000000' };
      }
      return { type: 'color', value: mode === 'green' ? '#00ff00' : '#000000' };
    },

    buildPublishedHtml(target) {
      const revision = Date.now();
      const rev = String(revision);
      const fallbackReloadScript = `(() => {
        const currentRev = document.body.getAttribute('data-publish-rev') || '';
        const revPath = (window.location.pathname || '').replace(/\\.html$/i, '.rev');
        const check = async () => {
          try {
            const response = await fetch(revPath + '?_check=' + Date.now(), { cache: 'no-store' });
            if (!response.ok) return;
            const text = (await response.text()).trim();
            if (text && text !== currentRev) {
              window.location.reload();
            }
          } catch {
            // ignore transient network or parser errors
          }
        };
        setInterval(check, 1500);
      })();`;
      if (target?.type === 'url' && target.value) {
        const src = escapeHtmlAttr(addCacheBustParam(target.value, revision));
        return {
          html: `<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><style>html,body{margin:0;width:100%;height:100%;overflow:hidden;background:#000;}iframe{border:0;width:100%;height:100%;display:block;}</style></head><body data-publish-rev="${escapeHtmlAttr(rev)}"><iframe src="${src}" allow="autoplay; fullscreen"></iframe><script>${fallbackReloadScript}</script></body></html>`,
          revision: rev
        };
      }
      const color = escapeHtmlAttr(target?.value || '#000000');
      return {
        html: `<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><style>html,body{margin:0;width:100%;height:100%;overflow:hidden;background:${color};}</style></head><body data-publish-rev="${escapeHtmlAttr(rev)}"><script>${fallbackReloadScript}</script></body></html>`,
        revision: rev
      };
    },

    writePublishedScreenFile(AppContext, target, reason = 'default') {
      const info = this.getPublishFileInfo(AppContext);
      if (!info) return null;
      fs.mkdirSync(path.dirname(info.filePath), { recursive: true });
      const built = this.buildPublishedHtml(target);
      fs.writeFileSync(info.filePath, built.html, 'utf8');
      fs.writeFileSync(info.revFilePath, `${built.revision}\n`, 'utf8');
      AppContext.log(`URL Publish (${reason}) wrote ${info.relativePath}`);
      return info;
    },

    removePublishedScreenFile(AppContext) {
      const info = this.getPublishFileInfo(AppContext);
      if (!info) return;
      if (fs.existsSync(info.filePath)) {
        fs.unlinkSync(info.filePath);
        AppContext.log(`URL Publish removed ${info.relativePath}`);
      }
      if (fs.existsSync(info.revFilePath)) {
        fs.unlinkSync(info.revFilePath);
      }
    },

    syncPublishedScreenDefault(AppContext, additionalScreens = null) {
      const screens = Array.isArray(additionalScreens)
        ? additionalScreens
        : this.normalizeAdditionalScreens(AppContext?.config?.additionalScreens);
      const publishEntries = this.getPublishEntries(screens);
      if (!publishEntries.length) {
        try {
          this.removePublishedScreenFile(AppContext);
        } catch (err) {
          AppContext.error(`Failed to remove URL publish file: ${err.message}`);
        }
        return null;
      }
      const target = this.resolveDefaultPublishedTarget(AppContext, publishEntries[0]);
      try {
        return this.writePublishedScreenFile(AppContext, target, 'default');
      } catch (err) {
        AppContext.error(`Failed to write URL publish default file: ${err.message}`);
        return null;
      }
    },

    publishToUrlScreen(AppContext, sourceUrl, additionalScreens = null) {
      const screens = Array.isArray(additionalScreens)
        ? additionalScreens
        : this.normalizeAdditionalScreens(AppContext?.config?.additionalScreens);
      const publishEntries = this.getPublishEntries(screens);
      if (!publishEntries.length || !sourceUrl) return null;
      const publishConfig = publishEntries[0];
      const resolvedUrl = applyPresentationOptions(sourceUrl, AppContext.config, {
        language: publishConfig.language || (AppContext.config.preferredPresentationLanguage || AppContext.config.language || 'en'),
        variant: publishConfig.variant || ''
      });
      try {
        return this.writePublishedScreenFile(AppContext, { type: 'url', value: resolvedUrl }, 'publish');
      } catch (err) {
        AppContext.error(`Failed to write URL publish file: ${err.message}`);
        return null;
      }
    },

    syncUrlPublishForConfig(AppContext) {
      try {
        return this.syncPublishedScreenDefault(AppContext);
      } catch (err) {
        AppContext.error(`Failed to sync URL publish file: ${err.message}`);
        return null;
      }
    },
    ...
    async showDefaultOnAdditionalScreens(AppContext) {
      ...
      if (!additionalScreens.length) {
        ...
        this.syncPublishedScreenDefault(AppContext, additionalScreens);
        return;
      }
    ...
    async openAdditionalScreensForPeerUrl(AppContext, url) {
      ...
      this.ensureAdditionalScreenWindows(AppContext, additionalScreens);
      this.publishToUrlScreen(AppContext, url, additionalScreens);
    ...
    closeWindow(options = {}) {
        ...
        this.unregisterGlobalHotkeys();
        if (this.presWindow && !this.presWindow.isDestroyed()) {
            // forceClose skips the fade-to-black close event (e.g. app shutdown).
            // Normal close goes through the 'close' event handler which fades first.
            if (forceClose) {
                this.presWindow.destroy();
            } else {
